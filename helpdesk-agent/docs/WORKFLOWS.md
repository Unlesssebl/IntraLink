...
| `/duplicates` `[N]`<br>`/dedup` | `найди дубликаты`, `почисти дубли`, `повторные заявки` | Автоматический поиск, группировка и пакетная отмена дубликатов в очереди 1-й линии (Статус 30: **Отменена** + ссылка на #Master) |
...
| `/newuser <ID>` | `создай пользователя <ID>`, `создай уз <ID>` | Автоматическое создание пользователя в Active Directory по данным заявки с генерацией пароля, добавлением в группу `HLP_<Организация>` и закрытием (Статус 29: **Выполнена**) |
| `/wlan <ID>` | `выдай wifi <ID>`, `вайфай <ID>` | Автоматическая выдача доступа Wi-Fi в AD (группа `WLAN-WORKNET`) и закрытие (Статус 29: **Выполнена**) |
...
**Что делает агент:**
1. **Сквозной аудит монорепозитория:** анализирует конфигурации зависимостей (`requirements.txt`, `pyproject.toml`), манифест [`docs/architecture.md`](../../docs/architecture.md) и профили I/O воркеров.
