...
> Продуктовые границы текущего Web-этапа, роли Rule Engine / RAG / AI и поведение интерфейса определены в [единой концепции](web-autopilot/product-concept.md). Этот архитектурный обзор не является подтверждением реализации всех перечисленных целевых возможностей. Следующий этап согласования описан в [карте доведения](web-autopilot/concept-alignment.md).
...
### 7.4. Устойчивый цикл заявки и неопределённый результат

Ручной режим и автопилот используют единый `TicketRun` в PostgreSQL. Команды управляемой заявки обязаны ссылаться на активный цикл; прямой triage и legacy Command API не могут параллельно изменить такую заявку. Назначение сервисной учётной записи `assistant` открытой заявке регистрирует основание запуска, но не заменяет политику подтверждения команды.

Execution Worker возвращает структурированные `failure_kind`, `failure_code` и `verified_failure`. Только проверенный отказ исполнения допускает отмену заявки по шаблону. Сбой инфраструктуры, отсутствие проверки или потеря ответа переводят команду либо цикл в `needs_review`/`system_error` без повторного изменяющего действия.

Redis доставляет команды как транспорт, а PostgreSQL хранит их состояние и lease. Повторная доставка во время активного lease не подтверждается. После истечения lease команда с неизвестным внешним результатом фиксируется как `needs_review`; оператор сверяет фактическое состояние перед продолжением.

Poller читает активные циклы страницами по стабильному ключу, ограничивает параллелизм и изолирует ошибку одной заявки от остальных. Параметры страницы и параллелизма задаются `AUTOPILOT_POLL_BATCH_SIZE` и `AUTOPILOT_MAX_CONCURRENCY`.

### 7.5. Декларативный сценарный оркестратор (TicketRunOrchestrator)

Система переведена с процедурного монолита на модульный декларативный автомат состояний `TicketRunOrchestrator`:
- **Единый реестр сценариев (`ScenarioRegistry`):** сценарии `install_printer`, `create_user`, `grant_wlan`, `redirect`, `offline_host` с нормализацией ключей и валидацией контрактов.
- **Извлечение фактов (`FactExtractor`):** объединение наблюдений из полей тикета, текста и комментариев заявителя с приоритетом источника (`source_precedence`) и иммунитетом к LLM-галлюцинациям.
- **Многоконтурная раскатка (Rollout Pipeline):**
  - `shadow`: пассивное фоновое сопоставление решений (`ShadowComparator`) без побочных эффектов с записью расхождений и метрик в `GET /api/v2/autopilot/shadow/metrics`.
  - `canary`: детерминированный выбор тикетов по стабильному хэшу `task_id` (`canary_percent`: 1–100%).
  - `active`: промышленное исполнение 100% потока заявок через оркестратор.
  - `Emergency Rollback`: мгновенный перевод сценариев в безопасный режим `shadow` в 1 клик через API/UI.
- **Инвариант финализации тикетов:** воркер среды Windows (`execution-worker`) возвращает строго структурированное доказательство исполнения (`ExecutionResult` в Redis Streams). Изменение статусов (`29`/`30`), списание трудозатрат и отправка комментариев заявителю выполняются **исключительно** Core API через `ResolutionPolicy` в методе `_reconcile_command`.
