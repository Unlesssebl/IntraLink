...
2. **Сетевая доступность:** Выполняется пинг целевого ПК и МФУ (для сетевого подключения `tcpip`). Если устройства недоступны, процесс прерывается, а заявка переводится в статус «Требует уточнения» с уведомлением инициатора, минуя этап подтверждения администратором.
