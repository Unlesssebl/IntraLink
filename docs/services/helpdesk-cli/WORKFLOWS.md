...
>    Определяются через файлы навыков (`.agents/skills/intraservice-helpdesk/SKILL.md`) и системные правила (`helpdesk-cli/GEMINI.md`).
...
| `/reflect` | `саморефлексия`, `аудит скорости`, `оптимизируй память` | Оперативный анализ сессии, устранение лишних чтений файлов (Zero-Read) и обновление оперативной памяти агента |
| `/deep-reflect` | `глубокая рефлексия`, `/arch-review`, `/evolve` | Глубокий стратегический аудит архитектуры, ревизия зависимостей, оптимизация ресурсов и подготовка решений (ADR) |
...
**Что делает агент:**
1. Загружает полные данные по задаче через `helpdesk.py task 139022`.
...
**Что делает агент:**
1. Скачивает вложение задачи в локальную директорию через `helpdesk.py attachment <ID>`.
...
Используется для аудита производительности работы агента, устранения медленных или ошибочных паттернов и дополнения оперативной памяти (`helpdesk-cli/GEMINI.md`).
...
4. Предлагает и после подтверждения оператором точечно вносит оптимизации в `helpdesk-cli/GEMINI.md` и `templates.json`.

---

### Сценарий 6.1: Глубокая архитектурная рефлексия и эволюция системы (`/deep-reflect`)

Используется для стратегического пересмотра фундаментальных аспектов монорепозитория: архитектурных границ сервисов, ревизии тяжелых/устаревших зависимостей, оптимизации скрытых блокировок I/O, модели межсервисного обмена и повышения общей отказоустойчивости.

**Пример вызова:**
```text
/deep-reflect
/deep-reflect techstack
/deep-reflect perf
/deep-reflect comms
/deep-reflect resilience
/arch-review
проведи глубокую рефлексию архитектуры
```

**Что делает агент:**
1. **Сквозной аудит монорепозитория:** анализирует конфигурации зависимостей (`requirements.txt`, `pyproject.toml`), манифест [`docs/architecture.md`](../../architecture.md) и профили I/O воркеров.
2. **Многомерная оценка (5D Framework):** оценивает связность модулей, скорость и накладные расходы сериализации, блокировки `asyncio.loop`, пулы соединений и механизмы изоляции сбоев.
3. **Сравнительный анализ альтернатив:** готовит матрицу решений (Trade-off Matrix) с сопоставлением текущего решения и современных высокопроизводительных альтернатив (например, перенос тяжелых синхронных библиотек на нативные асинхронные драйверы, сжатие payloads, Redis Streams vs Pub/Sub).
4. **Генерация архитектурного отчета (ADR / RFC):** формирует четкий артефакт с описанием узких мест, обоснованием выбора и пошаговым планом безаварийной миграции (Zero-Downtime Rollout).
5. **Согласование с инженером:** презентует выводы оператору для утверждения плана инженерных работ.
