...
├── desktop-companion/ # Нативный Windows tray-helper (Tauri 2, Rust): вызов DameWare/LiteManager/RDP
...
### 10. Доказательность фактов (Evidence-Based Decisioning)
* **Правило:** Решение по заявке компилируется только из проверенных и типизированных фактов `FactBag` с контролем источника правды (Provenance). LLM-экстракция фактов работает только в роли fallback и никогда не может переопределить явно переданные структурные данные заявки.
* **Причина:** Исключение галлюцинаций нейросети при определении ФИО, логинов, имен хостов и принтеров.

### 11. Неблокирующие пакетные операции (Non-blocking Batch Operations)
* **Правило:** Пакетный анализ очередей (`POST /api/v1/triage/analyze-batch`) и длительные операции никогда не удерживают синхронный HTTP-запрос. Сервер моментально отвечает `202 Accepted` с `batch_id`, а прогресс публикуется через Redis Pub/Sub и Server-Sent Events (SSE).
* **Причина:** Гарантированная защита от `504 Gateway Timeout` на корпоративных reverse proxy и сохранение отзывчивости интерфейса оператора.
