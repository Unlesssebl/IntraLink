...
1. **`account_create` (Создание учетной записи / Онбординг):**
   ...
   * *Безопасность:* Временный пароль **никогда не публикуется в открытый тикет**. Передается руководителю/HR через регламентный закрытый канал (защищенная корпоративная почта или скрытая служебная заметка `IsPrivateComment: true` с ограничением прав). Публичный комментарий в тикет содержит лишь уведомление о завершении создания учетки.
