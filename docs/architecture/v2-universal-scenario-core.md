...
* **Условие запуска:** Заявка поступила в очередь 1-й линии и не назначена на сервисного бота.
...
* **Сценарий оператора («Одобрительный толчок»):** Оператор открывает заявку в UI супервизора. План уже рассчитан на 100%. Оператор бегло проверяет корректность (1–2 секунды) и нажимает <kbd>Enter</kbd> («Одобрить»). Задача уходит в Taskiq, выполняется, и тикет закрывается.
...
1. **`account_create` (Создание учетной записи / Онбординг):**
   * *Сервисы:* Сервис 55 («Заявка на пользователя Directum»), Сервис 232 («Доступ к корпоративным системам»).
   * *Обязательные факты:* `first_name`, `last_name`, `department`, `title`, `phone`.
   * *Исполнение:* Детерминированная генерация логина (`ivanov.i`, при коллизии `ivanov.i2`), создание пользователя в целевом OU подразделения, принудительный флаг `pwdLastSet=0`, генерация криптографически стойкого временного пароля (`secrets.token_urlsafe(12)`).
   * *Безопасность:* Временный пароль **никогда не публикуется в открытый тикет**, а передается руководителю/HR по регламентному закрытому каналу.
...
| **11** | **Безопасность временного пароля** | Временный пароль от новой учетки не должен утекать в открытую историю тикета и рассылку. | **Zero-Plaintext Policy:** В тикет пароль не пишется. Передается руководителю/HR по регламентному закрытому каналу, выставляется `pwdLastSet=0`. В логах секрет маскируется (`***REDACTED***`). |
