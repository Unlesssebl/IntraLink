...
1. **[Сценарии автоматизации Helpdesk: Web UI Quick Wins & Пилоты](scenarios-roadmap.md)** — внедрение быстрых диагностических действий: бэкенд разблокировки УЗ в AD (`POST /api/v1/ad/users/{username}/unlock`), Spooler Doctor (сброс очереди печати), инспектор блокировок SMB (`Get-SmbOpenFile`), очистка диска C: и кэша 1С.
2. **[Гибридный каскад классификации намерений (Intent Routing Cascade)](intent-classification.md)** — замена хрупких регулярных выражений: `Regex Guard (Tier 1) ➔ FastEmbed Semantic Anchors BGE-M3 + NegationGuard (Tier 2) ➔ SLM Qwen-2.5 Intent Verifier (Tier 3) ➔ Cross-Encoder NLI (Tier 4)`.

### 📦 Реализованные и архивированные этапы (Внедрено в Baseline):
* **[Целостность решений и двухконтурная маршрутизация](hierarchical-routing-and-response-integrity-plan.md)** — 100% устранение пустых ответов (0 пустых на 101 заявке), миграция `20260912_0016` (политика и шаблон `peripheral_clarify`), нейтральный аварийный черновик `EMERGENCY_DRAFT_TEXT` и fail-closed блокировка шлюзов, BGE-M3 FastEmbed семантический подбор сценариев Pass 1 (`off`/`shadow`/`canary`/`on`), 643 passed тестов.
* **[Качество сценариев и аудит решений (Этапы 1–5)](ticket-scenario-quality-roadmap.md)** — детерминированный replay 99 контрольных заявок, нормализация фактов периферии и ПК, предметные диагностические планы инженера и ответы заявителю (`TruthfulnessGuard`), Capacity-Aware Context Budgeting (Ollama vs LiteLLM/Gemini), Single Grounded Answer и переключение тональности, двухфазное применение (`DecisionApplicationRequest`/`Attempt`), исключение сетевых повторов, сверка `unknown`, Levenshtein diff, UI-кнопка отклонения рекомендаций с причинами, сервис аналитики качества (`decision_quality_report.py`) и ревизия `ANALYSIS_REVISION=5`.
