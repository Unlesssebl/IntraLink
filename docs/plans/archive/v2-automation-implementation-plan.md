...
### 🧠 Спринт 6: Умный роутинг (Catalog-First Prior + Coherence Guard), расширение Core-6 и UI-консоль управления (Завершено ✅)
* **Цель:** Переход на многофакторный скоринг сценариев, исключение коллизий формулировок, расширение библиотеки действий до Core-6 и ввод в эксплуатацию веб-панели управления автопилотом.

#### Задачи:
1. **Многофакторный роутер сценариев (`worker/src/scenarios/router.py` и `contracts.py`):** ✅ Реализовано
   * Замена жесткого линейного перебора `can_handle()` на взвешенный скоринг `ScenarioRouter.route_task(task, scenarios) -> ScenarioMatch`.
   * **Catalog-First Prior:** априорный вес выбора услуги `ServiceId` и формы `TaskTypeId` (Confidence $P_0 \ge 0.90$).
   * **Coherence Guard (Барьер противоречий):** детекция кросс-доменных расхождений (`COHERENT` vs `DIVERGENT`) между выбранной услугой и текстом тикета.
2. **Расширение библиотеки сценариев до Core-6 (`worker/src/scenarios/`):** ✅ Реализовано
   * `grant_wlan`: выдача доступа Wi-Fi (добавление учетной записи в доменную группу `WLAN-WORKNET` с Single-DC Affinity).
   * `service_redirect`: регламентная отмена и перенаправление заявок, ошибочно поданных в чужой сервис (Шлюз #1, статус 30).
   * `offline_host`: расширенная сокетная диагностика и оформление наряда на физический осмотр инженером (каб. 112).
3. **Операторская веб-консоль «Автопилот» (`web/src/features/autopilot/`):** ✅ Реализовано
   * **Живой мониторинг:** журнал исполнения команд на базе `CommandRecord` с коротким опросом (Short-Polling 1.5с) через TanStack Query v5.
   * **Управление матрицей автономии:** тумблеры сценариев (`FULL_AUTO`, `ASSISTED`, `DISABLED`) с нулевой задержкой через `PUT /api/v2/autopilot/policies/{scenario_key}`.
   * **Индикатор Circuit Breaker:** баннеры аномалий и кнопка ручного сброса предохранителя (*Reset Tripwire*).
   * **Аналитика эффективности:** счетчики автономно закрытых тикетов и сэкономленных человеко-часов Helpdesk.
4. **Ликвидация слепых зон и надежность рантайма (`worker/src/services/` и `worker/src/tasks/autopilot.py`):** ✅ Реализовано
   * **Анализатор намерений ответов заявителя (`UserReplyIntentAnalyzer`):**
     - Распознавание явного отказа (`cancel_request` ➔ авто-отмена Статус 30 без лишних раундов);
     - Распознавание встречных вопросов (`clarification_question` ➔ подсказка где найти WKS/IP);
     - Проверка наличия вложений (`attachments_only` ➔ эскалация человеку при фото наклейки/скриншоте).
   * **Валидатор корпоративных подсетей:**
     - Отсечение домашних адресов (`192.168.0.0/16`, `127.0.0.0/8`, `169.254.0.0/16`) с вежливым разъяснением.
   * **Распределенная блокировка задач (In-Flight Task Concurrency Lock):**
     - Distributed lock в Redis `lock:autopilot:{task_id}` с TTL 60с для исключения дублирующих запусков и гонок воркеров.
   * **Безопасный транзит статусов:**
     - Гарантированный двухшаговый перевод `6 ➔ 2 ➔ 3` при закрытии тикета из паузы.
5. **E2E Верификация и стресс-тестирование:** ✅ Пройдено (133 теста, 100% зеленые)
   * Все 133 integration и unit-теста проходят без ошибок (`133 passed in 7.10s`).
   * Сборка веб-панели `npm run build` завершается чисто (`built in 3.29s`).
   * Линтер `ruff check .` — без единого замечания.

* **DoD:** Полностью достигнут. Точность маршрутизации обращений $\ge 98\%$, ликвидированы все 6 краевых случаев, 6 активных сценариев, операторы полноценно управляют автопилотом из UI, а сбои внешних систем изолируются предохранителем без деградации конвейера.
