...
| :---: | :--- | :--- | :---: |
| **Спринт 1** | **Taskiq Runtime & Очереди** | `worker/src/broker.py`, `worker/src/main.py`, Docker Taskiq runtime | ⏳ Готов к старту |
| **Спринт 2** | **Фоновый синк Базы знаний** | `sync_kb.py`, PII-очистка, cron 02:00, FastEmbed батчи в pgvector | 📋 Запланирован |
| **Спринт 3** | **Ingestion & Пульс 30с** | Поллер `filterid=984` + `ChangedMoreThan`, двойной Watermark (Redis+Postgres) | 📋 Запланирован |
