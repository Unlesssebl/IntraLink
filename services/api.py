from datetime import datetime
import aiohttp
import base64
import logging
from config import INTRAService_URL

# Disable SSL verification for internal domains if needed
SSL_VERIFY = False

logger = logging.getLogger(__name__)

def parse_api_date(date_str):
    """
    Парсит дату из API IntraService. 
    Поддерживает форматы: YYYY-MM-DD HH:MM:SS, YYYY-MM-DDTHH:MM:SS, DD.MM.YYYY HH:MM:SS
    """
    if not date_str:
        return None
    
    date_str = date_str.replace("T", " ")
    
    formats = [
        "%Y-%m-%d %H:%M:%S",
        "%Y-%m-%d %H:%M",
        "%d.%m.%Y %H:%M:%S",
        "%d.%m.%Y %H:%M"
    ]
    
    for fmt in formats:
        try:
            return datetime.strptime(date_str, fmt)
        except ValueError:
            continue
    
    logger.warning(f"Failed to parse date: {date_str}")
    return None

async def _make_request(endpoint, method="GET", auth_b64=None, auth=None, params=None, json_data=None):
    """
    Универсальная функция для выполнения HTTP-запросов к API IntraService.
    """
    url = f"{INTRAService_URL}{endpoint}"
    
    headers = {"Content-Type": "application/json"}
    if auth_b64:
        headers["Authorization"] = f"Basic {auth_b64}"

    connector = aiohttp.TCPConnector(ssl=SSL_VERIFY)
    try:
        async with aiohttp.ClientSession(connector=connector) as session:
            async with session.request(
                method=method,
                url=url,
                headers=headers,
                auth=auth,
                params=params,
                json=json_data,
                timeout=aiohttp.ClientTimeout(total=30)
            ) as response:
                if response.status == 200:
                    return await response.json()
                else:
                    text = await response.text()
                    logger.error(f"API Error [{response.status}] for {endpoint}: {text}")
                    return None
    except Exception as e:
        logger.exception(f"Network error during API request to {endpoint}: {e}")
        return None

async def get_tasks(auth_b64, filters=None):
    params = {
        "include": "status"
    }
    if filters:
        params.update(filters)
        
    return await _make_request(
        endpoint="task",
        method="GET",
        auth_b64=auth_b64,
        params=params
    )

async def get_task_lifetime(auth_b64, task_id):
    return await _make_request(
        endpoint="tasklifetime",
        method="GET",
        auth_b64=auth_b64,
        params={"taskid": task_id}
    )

async def get_statuses(auth_b64):
    return await _make_request(
        endpoint="taskstatus",
        method="GET",
        auth_b64=auth_b64
    )

async def verify_credentials(login, password):
    auth = aiohttp.BasicAuth(login, password)
    
    response_data = await _make_request(
        endpoint="user",
        method="GET",
        auth=auth,
        params={"getcurrentuserinfo": "true"}
    )
    
    if response_data is not None:
         # Return the base64 string to store in DB for subsequent requests
        auth_str = f"{login}:{password}"
        auth_b64 = base64.b64encode(auth_str.encode()).decode()
        user_id = response_data.get("Id")
        return auth_b64, user_id
    return None, None
