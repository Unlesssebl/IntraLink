...
import { fetchQueue, mapStatusIdToStatus, getSafeStatusName, getBatchStatus, cancelBatch } from './lib/tasks';

import { useLiveEvents, type TaskAnalyzedEvent } from './hooks/useLiveEvents';
...
function MainApp() {
  ...
  const [activeBatch, setActiveBatch] = useState<ActiveBatchState | null>(null);
  const analysisBufferRef = useRef<Map<number, TaskAnalyzedEvent>>(new Map());
  ...
  const flushAnalysisBuffer = useCallback(() => {
    ...
    setTickets(prev =>
      prev.map(t => {
        const ev = updates.get(t.rawId);
        if (!ev) return t;

        const analysis = ev.analysis;
        const isProcessed = ev.status === 'processed' || ev.status === 'skipped';
        return {
          ...t,
          isProcessed,
          scenarioKey: ev.scenario_key || t.scenarioKey,
          envelope: analysis?.envelope || t.envelope,
          analysis: analysis || t.analysis,
          aiSuggestion: analysis?.suggested_action?.comment || t.aiSuggestion,
          targetStatusId: analysis?.suggested_action?.target_status_id || t.targetStatusId,
          targetStatusName: analysis?.suggested_action?.target_status_name || t.targetStatusName,
          isRedirect: analysis?.is_redirect ?? t.isRedirect,
          isDuplicate: analysis?.is_duplicate ?? t.isDuplicate,
        };
      })
    );
  }, []);
  ...
  const { isConnected: isLiveConnected } = useLiveEvents({
    ...
    onTaskStatusUpdated: (taskIds, newStatusId) => {
      ...
      const targetStatus = mapStatusIdToStatus(newStatusId);

      const targetStatusName = getSafeStatusName(newStatusId);
      ...
      setTickets(prev =>
        ...
        prev.map(t =>
          ...
          idSet.has(t.id) || idSet.has(String(t.rawId))
            ...
            ? { ...t, status: targetStatus, statusId: newStatusId, statusName: targetStatusName }
