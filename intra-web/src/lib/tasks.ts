...
export function mapStatusIdToStatus(statusId: number, statusName?: string): Status {
  ...
  if (statusId === 2 || statusId === 26 || statusId === 27 || (statusName && /работе/i.test(statusName))) return 'in_progress';



  if (statusId === 3 || statusId === 10 || statusId === 35 || statusId === 36 || statusId === 37 || statusId === 48 || (statusName && /ожидан|отложен|уточнен|поставк|пользовател|планов/i.test(statusName))) return 'waiting';
...
export function getSafeStatusName(statusId: number, statusName?: string): string {



  if (statusName && statusName.trim()) return statusName.trim();
...
export function mapTaskToTicket(task: TaskItem): Ticket {
  ...
  const isProcessed = Boolean(analysis ? analysis.has_result : false);
  ...
  return {
    ...
    statusName: getSafeStatusName(task.status_id, task.status_name),
