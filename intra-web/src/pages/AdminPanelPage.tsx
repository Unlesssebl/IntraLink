...
import {
  ...
  fetchTriageAnalysisStats,
  resetTriageAnalysis,
  type TriageAnalysisStats,
} from '../lib/adminApi';
...
export default function AdminPanelPage({ theme = 'light' }: AdminPanelPageProps) {
  ...
  // Triage Analysis Lifecycle & Cache State
  const [analysisStats, setAnalysisStats] = useState<TriageAnalysisStats | null>(null);
  const [loadingAnalysisStats, setLoadingAnalysisStats] = useState(false);
  const [isResetAnalysisModalOpen, setIsResetAnalysisModalOpen] = useState(false);
  const [resetAnalysisScope, setResetAnalysisScope] = useState<'all' | 'cache_only'>('all');
  const [resetAnalysisConfirmed, setResetAnalysisConfirmed] = useState(false);
  ...
  const loadAnalysisStats = useCallback(async () => {
    if (!token) return;
    setLoadingAnalysisStats(true);
    try {
      const stats = await fetchTriageAnalysisStats(token);
      setAnalysisStats(stats);
    } catch (err: any) {
      console.error('Не удалось загрузить статистику анализа:', err);
    } finally {
      setLoadingAnalysisStats(false);
    }
  }, [token]);
  ...
  }, [token, activeTab, loadAiHealthData, loadAnalysisStats]);

  const handleResetAnalysis = async () => {
    if (!token) return;
    if (resetAnalysisScope === 'all' && !resetAnalysisConfirmed) return;
    setResettingAnalysis(true);
    setStatusMessage(null);
    try {
      const isFull = resetAnalysisScope === 'all';
      const res = await resetTriageAnalysis(token, {
        purge_decisions: isFull,
        purge_ai_cache: true,
        purge_locks: isFull,
      });
      setStatusMessage({
        type: 'success',
        text: res.message || 'Сброс анализа успешно выполнен',
      });
      setIsResetAnalysisModalOpen(false);
      setResetAnalysisConfirmed(false);
      ...
      setStatusMessage({
        type: 'error',
        text: err.message || 'Ошибка сброса анализа заявок',
      });
    } finally {
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-neutral-100 font-sans flex flex-col">
      ...
      <div className="flex-1 max-w-5xl w-full mx-auto p-6 md:p-8 space-y-6">
        ...
        {activeTab === 'ai-hub' && (
          <div className="space-y-6">
            ...
            {/* Управление жизненным циклом и кэшем анализа заявок */}
            <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 shadow-sm space-y-5 mt-6">
              <div className="flex items-center justify-between flex-wrap gap-3 border-b border-neutral-800 pb-4">
                <div>
                  <h3 className="text-sm font-semibold text-neutral-100 flex items-center gap-2">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-blue-400">
                      <path d="M21.5 2v6h-6M21.34 15.57a10 10 0 1 1-.57-8.38l5.67-5.67"/>
                    </svg>
                    <span>Жизненный цикл анализа заявок и кэш ответов</span>
                    {loadingAnalysisStats && <span className="text-xs text-blue-400 animate-pulse font-normal">(обновление...)</span>}
                  </h3>
                  <p className="text-xs text-neutral-400 mt-1 max-w-2xl leading-relaxed">
                    База данных PostgreSQL хранит постоянные карточки решений (<code className="text-blue-400 font-mono">decision_records</code>), а Redis кэширует сгенерированные ответы нейросетей (<code className="text-blue-400 font-mono">cache:ai:*</code>). Полный сброс удаляет ранее зафиксированные вердикты и кэш, возвращая очередь в состояние «Ожидает анализа» для повторной обработки на актуальных моделях.
                  </p>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    disabled={loadingAnalysisStats}
                    className="px-3 py-1.5 text-xs font-medium text-neutral-300 bg-neutral-800 hover:bg-neutral-700 rounded-lg border border-neutral-700 transition-colors flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
                  >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div className="bg-neutral-950/80 border border-neutral-800/80 rounded-xl p-3.5 space-y-1">
                  <span className="text-[11px] text-neutral-400 block font-medium">Решений в БД:</span>
                  <div className="flex items-baseline gap-1.5">
                    <span className="text-lg font-bold text-neutral-100 font-mono">
                      {analysisStats?.total_decisions ?? '—'}
                    </span>
                    <span className="text-[10px] text-neutral-500 font-mono">
                      (вариантов: {analysisStats?.total_variants ?? 0})
                    </span>
                  </div>
                  <span className="text-[10.5px] text-neutral-500 block">таблица decision_records</span>
                </div>

                <div className="bg-neutral-950/80 border border-neutral-800/80 rounded-xl p-3.5 space-y-1">
                  <span className="text-[11px] text-neutral-400 block font-medium">Кэш ответов AI:</span>
                  <div className="flex items-baseline gap-1.5">
                    <span className="text-lg font-bold text-emerald-400 font-mono">
                      {analysisStats?.cached_ai_responses ?? '—'}
                    </span>
                    <span className="text-[10px] text-neutral-500">ключей</span>
                  </div>
                  <span className="text-[10.5px] text-neutral-500 block">Redis (cache:ai:*)</span>
                </div>

                <div className="bg-neutral-950/80 border border-neutral-800/80 rounded-xl p-3.5 space-y-1">
                  <span className="text-[11px] text-neutral-400 block font-medium">Активные блокировки:</span>
                  <div className="flex items-baseline gap-1.5">
                    <span className="text-lg font-bold text-neutral-200 font-mono">
                      {analysisStats?.active_locks ?? '—'}
                    </span>
                    <span className="text-[10px] text-neutral-500">fences/locks</span>
                  </div>
                  <span className="text-[10.5px] text-neutral-500 block">Redis распределенные замки</span>
                </div>

                <div className="bg-neutral-950/80 border border-neutral-800/80 rounded-xl p-3.5 space-y-1">
                  <span className="text-[11px] text-neutral-400 block font-medium">Ревизия схемы:</span>
                  <div className="flex items-baseline gap-1.5">
                    <span className="text-xs font-bold text-purple-400 font-mono truncate">
                      {analysisStats?.analysis_revision || '20260912_0016'}
                    </span>
                  </div>
                  <span className="text-[10.5px] text-neutral-500 block">CURRENT_SCHEMA_REVISION</span>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between flex-wrap gap-3 pt-2">
                <div className="text-xs text-neutral-400">
                  <span>Доступны операции выборочного или полного сброса данных триажа:</span>
                </div>
                ...
                <div className="flex items-center gap-2.5 flex-wrap">
                  <button
                    type="button"
                    onClick={() => {
                      setResetAnalysisScope('cache_only');
                      setIsResetAnalysisModalOpen(true);
                      setResetAnalysisConfirmed(false);
                    }}
                    disabled={resettingAnalysis}
                    className="px-3.5 py-2 bg-neutral-800 hover:bg-neutral-700 text-neutral-200 rounded-xl text-xs font-medium border border-neutral-700 transition-colors cursor-pointer flex items-center gap-2"
                  >
                    ...
                    <span>Очистить кэш ответов AI (Redis)</span>
                  </button>

                  <button
                    type="button"
                    onClick={() => {
                      setResetAnalysisScope('all');
                      setIsResetAnalysisModalOpen(true);
                      setResetAnalysisConfirmed(false);
                    }}
                    disabled={resettingAnalysis}
                    className="px-4 py-2 bg-rose-600/90 hover:bg-rose-500 text-white rounded-xl text-xs font-semibold shadow-lg shadow-rose-950/40 transition-all cursor-pointer flex items-center gap-2"
                  >
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    ...
                    <span>Полный сброс анализа для всех заявок</span>
                  </button>
                </div>
              </div>
            </div>
      ...
      {/* Модальное окно подтверждения сброса анализа заявок */}
      {isResetAnalysisModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-neutral-900 border border-neutral-800 rounded-2xl max-w-md w-full p-6 space-y-4 shadow-2xl animate-in fade-in zoom-in-95 duration-150">
            <div className="flex items-center gap-3 text-rose-400">
              <div className="w-10 h-10 rounded-xl bg-rose-500/10 border border-rose-500/20 flex items-center justify-center shrink-0">
              ...
              <div>
                <h3 className="text-base font-bold text-neutral-100">
                  {resetAnalysisScope === 'all'
                    ? 'Полный сброс анализа для всех заявок'
                    : 'Сброс L2-кэша ответов AI'}
                </h3>
                <p className="text-xs text-neutral-400">
                  {resetAnalysisScope === 'all'
                    ? 'Удаление всех решений триажа и очистка кэша'
                    : 'Удаление закэшированных текстов ответов LLM'}
                </p>
              </div>
            </div>
            ...
            {resetAnalysisScope === 'all' ? (
              <div className="space-y-2.5 text-xs text-neutral-300 leading-relaxed">
                <p>
                  Вы собираетесь выполнить <strong className="text-rose-400">полный сброс анализа</strong>:
                </p>
                <ul className="list-disc list-inside space-y-1 text-neutral-400 pl-1 text-[11.5px]">
                  <li>Будет удалено <strong className="text-neutral-200 font-mono">{analysisStats?.total_decisions ?? 0}</strong> зафиксированных решений из PostgreSQL (<code className="text-blue-400 font-mono">decision_records</code>).</li>
                  <li>Будут сброшены все сгенерированные варианты ответов (<code className="text-blue-400 font-mono">decision_response_variants</code>).</li>
                  <li>Будет очищен L2-кэш генераций AI в Redis (<code className="text-blue-400 font-mono">cache:ai:*</code>, {analysisStats?.cached_ai_responses ?? 0} ключей).</li>
                  <li>Будут сняты все распределенные блокировки и очереди триажа.</li>
                </ul>
                <p className="text-neutral-400 text-[11px] pt-1">
                  Все заявки в очереди вернутся в исходное состояние «Ожидает анализа». При открытии или пакетном разборе они будут заново проанализированы по актуальным правилам.
                </p>

                <label className="flex items-start gap-2.5 p-3 rounded-xl bg-neutral-950/80 border border-neutral-800 text-xs text-neutral-200 cursor-pointer mt-3">
                  <input
                    type="checkbox"
                    checked={resetAnalysisConfirmed}
                    onChange={e => setResetAnalysisConfirmed(e.target.checked)}
                    className="mt-0.5 rounded border-neutral-700 text-rose-600 focus:ring-rose-500 bg-neutral-900"
                  />
                  <span>Я подтверждаю удаление всех сохраненных решений и полный сброс анализа</span>
                </label>
              </div>
            ) : (
              <div className="space-y-2 text-xs text-neutral-300 leading-relaxed">
                <p>
                  Будет очищен L2-кэш сгенерированных ответов нейросетей в Redis (<strong className="text-emerald-400 font-mono">cache:ai:*</strong>, {analysisStats?.cached_ai_responses ?? 0} записей).
                </p>
                <p className="text-neutral-400 text-[11.5px]">
                  Структура решений в БД сохранится, но при запросе повторного анализа или генерации тонов ответов система запросит свежий инференс у LiteLLM / Gemini / Ollama без использования кэша.
                </p>
              </div>
            )}
            ...
            <div className="flex items-center justify-end gap-2.5 pt-2 border-t border-neutral-800">
              ...
              <button
                type="button"
                disabled={(resetAnalysisScope === 'all' && !resetAnalysisConfirmed) || resettingAnalysis}
              ...
              >
                {resettingAnalysis ? (
                  <>
                    ...
                    <span>Выполняется сброс...</span>
                  </>
                ) : resetAnalysisScope === 'all' ? (
                  'Сбросить анализ всех заявок'
                ) : (
