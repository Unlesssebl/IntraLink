...
export const HostBadge: React.FC<HostBadgeProps> = ({ host, autoCheck = false }) => {
  ...
  if (!host) {
    return <span className="text-slate-500 text-xs italic font-mono">—</span>;
  }
    ...
    <div className="relative inline-block font-mono">
      ...
      {popoverOpen && (
        <>
          ...
          />
          <div className="absolute left-0 mt-1 z-50 w-56 bg-[#161a23] border border-[#2a3040] rounded-md shadow-xl p-3 text-xs space-y-2 animate-in fade-in zoom-in-95 duration-100">
            <div className="flex items-center justify-between border-b border-[#252b39] pb-1.5 font-semibold text-slate-200">
            ...
            {error ? (
              ...
              <div className="space-y-1 text-[11px] text-slate-300">
                ...
                <div className="flex justify-between">
                  <span className="text-slate-400">SMB (порт 445):</span>
                  <span
                    className={
                      data.smb_port_open ? "text-emerald-400" : "text-slate-500"
                    }
                  >
                ...
                <div className="flex justify-between">
                  <span className="text-slate-400">WinRM (5985):</span>
                  <span
                    className={
                      data.winrm_port_open ? "text-emerald-400" : "text-slate-500"
                    }
                  >
              ...
              <p className="text-[11px] text-slate-400">Загрузка данных...</p>
            )}
          </div>
        </>
      )}
