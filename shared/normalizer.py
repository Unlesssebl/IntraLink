...
_KEYBOARD_EN = "qwertyuiop[]asdfghjkl;'zxcvbnm,./QWERTYUIOP{}ASDFGHJKL:" + chr(34) + "ZXCVBNM<>?"
...
def _try_fix_prefix(prefix: str, known_prefixes: list[str]) -> str | None:
    ...
    for known in known_prefixes:
        # Не сопоставляем длинные префиксы с 2-буквенными филиальными
...
def resolve_pc_candidates(raw_name: str | None, company: str = "", dept: str = "") -> list[str]:
    ...
    # 2. КЗМК ТЭМПО
        ...
        candidates.extend([f"KZM{digits}", f"KZ{digits}", f"KZMK{digits}", f"NTEMW{digits}"])
    # 3. КМК ТЭМПО
    ...
    # 4. ТЭМПО-Логистик
        ...
        candidates.extend([f"TLK{digits}", f"TL{digits}", f"TKT{digits}", f"NTEMW{digits}"])
    # 5. Трубная компания ТЭМПО (НТЗ, ИТЗ, ТД ТЭМ)
    elif any(k in comp_lower for k in ["трубная", "нтз", "итз", "тд тэм", "трубный"]):
        candidates.extend([f"TKT{digits}", f"NT{digits}", f"NTZ{digits}", f"NTEMW{digits}"])
    # 6. ПТФК Технотрон
    ...
    # 9. Группа компаний ТЭМПО (различные ИП)
...
def resolve_printer_candidates(raw_name: str | None, company: str = "", dept: str = "") -> list[str]:
    """
