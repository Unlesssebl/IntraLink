...
class CoreAPIClient:
    ...
    async def _make_request(
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.request(
                    ...
                    if response.status in (200, 201):
                        ...
                        logger.warning("Ресурс не найден [%d] для %s", response.status, endpoint)
                        return None
                    else:
            ...
            logger.exception("Сетевая ошибка при обращении к Core API %s: %s", endpoint, e)
            return None

    async def login(self, tg_user_id: int, login: str, password: str) -> Optional[Dict[str, Any]]:
        """
