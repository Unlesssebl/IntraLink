...
def get_approval_keyboard(task_id: int) -> InlineKeyboardMarkup:
    buttons = [
        [InlineKeyboardButton(text="Подтвердить автоустановку", callback_data=f"printer_approve:{task_id}")],
        [InlineKeyboardButton(text="Отклонить (установка вручную)", callback_data=f"printer_reject:{task_id}")],
        [InlineKeyboardButton(text="Изменить параметры", callback_data=f"printer_edit:{task_id}")],
