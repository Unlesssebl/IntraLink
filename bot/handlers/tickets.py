...
async def build_tickets_page(
    tg_user_id: int, user_id: int, page: int = 1, pagesize: int = 5
):
    ...
    # Формируем фильтры
    filters = {"page": page, "pagesize": pagesize, "ExecutorIds": str(user_id)}
    ...
    text += "<i>(Вы назначены исполнителем)</i>\n\n"
...
async def cmd_mytickets(message: types.Message):
    ...
    if not user or not user.get("is_login"):
        ...
        await message.answer(
            "Вы не авторизованы.", reply_markup=get_main_keyboard(False)
        )
        return
    ...
    if not user_id:
        await message.answer(
            "❌ Ошибка: ваш внутренний ID не найден. Пожалуйста, перезайдите через Выйти -> Авторизация"
        )
        return
...
async def process_tickets_pagination(
    callback: types.CallbackQuery, callback_data: TicketsPagination
):
    ...
    user_id = user.get("is_user_id")
    text, reply_markup = await build_tickets_page(
        callback.from_user.id, user_id, page=callback_data.page
    )
