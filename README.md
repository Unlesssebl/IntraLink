...
Проект построен на базе модульного монорепозитория с единым ядром исполнения: интерфейсные слои (Telegram-бот, встроенный SPA `admin-ui`), шлюз интеграции (`core-api`), автономный инженерный кокпит (`helpdesk_agent`) и локальный AI-инференс (`ollama`).
...
4. **Admin UI (Vue 3 + Vite)** — современная панель управления (Single-File SPA), компилируемая на этапе Multi-stage Docker build и раздаваемая прямо из Core API.
...
```text
├── admin-ui/                  # Фронтенд панели администратора (Vue 3, Vite, Tailwind CSS)
