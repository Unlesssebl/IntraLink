...
Проект — **микросервисный монорепозиторий**:
- `core-api/` — FastAPI (шлюз, хранилище состояния, фоновый воркер).
- `bot/` — aiogram 3.x (интерфейсный слой, делегирует логику в Core API).
- `helpdesk_agent/` — автономный CLI-агент Helpdesk в среде Antigravity (AGY).
