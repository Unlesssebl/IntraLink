...
class AIConfigUpdate(BaseModel):
    auto_reply_service_ids: list[int] = Field(
        ..., description="Список ID разделов для автоответов"
    )
...
async def get_ai_status():
    ...
    try:
        ...
        auto_reply_mode = (
            await r.get("config:auto_reply_mode") or settings.AUTO_REPLY_MODE
        )
...
async def update_ai_config(payload: AIConfigUpdate):
    ...
    """
    if payload.auto_reply_mode not in (
        "comment_only",
        "comment_and_wait",
        "comment_and_resolve",
    ):
    ...
    try:
        r = get_redis_client()
        await r.set(
            "config:auto_reply_service_ids", json.dumps(payload.auto_reply_service_ids)
        )
        ...
        logger.info(
            ...
            payload.auto_reply_service_ids,
            payload.auto_reply_mode,
        )
...
@router.post(
    "/admin/api/ai-worker/test-reply/{task_id}",
...
async def generate_test_reply(task_id: int):
    ...
    try:
        ...
        logger.exception(
            "Ошибка при генерации тестового автоответа для задачи #%d: %s", task_id, e
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
class RAGQuotasUpdate(BaseModel):
    ...
    global_quotas: dict[str, int] = Field(
        ..., description="Глобальные квоты по статусам, например {'28': 10, '30': 5}"
    )
    service_quotas: dict[str, dict[str, int]] = Field(
        ..., description="Индивидуальные квоты для услуг, например {'123': {'28': 5}}"
    )
...
async def trigger_rag_build(service_ids: list[int] | None = None):
    ...
    if await r.get("rag_build:running") == "true":
        return {
            ...
            "message": "Процесс перестроения базы RAG уже запущен.",
        }
    ...
    global_quotas = (
        json.loads(global_quotas_str) if global_quotas_str else {"28": 10, "30": 5}
    )
    ...
    try:
        ...
        return {
            "status": "success",
            "message": "Команда на перестроение базы RAG успешно отправлена в ai-worker.",
        }
    except Exception as e:
...
@router.get(
    "/admin/api/ai-worker/rag/examples", dependencies=[Depends(verify_admin_jwt)]
)
async def get_rag_examples(
    ...
    try:
        offset = (page - 1) * limit
        query = select(TaskKnowledgeBase).where(
        ...
        query = (
            query.order_by(TaskKnowledgeBase.task_id.desc()).offset(offset).limit(limit)
        )
...
@router.delete(
    "/admin/api/ai-worker/rag/examples/{task_id}",
    dependencies=[Depends(verify_admin_jwt)],
)
...
@router.post(
    "/admin/api/ai-worker/rag/quotas", dependencies=[Depends(verify_admin_jwt)]
)
