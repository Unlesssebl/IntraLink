...
def classify_target_service(text: str, current_service_id: int | None = None) -> tuple[str | None, str | None]:
    ...
    # Проверка на запрос переустановки ОС (1-я линия, не 1С)
    is_os_reinstall = any(w in t for w in [
        "переустановка операционной системы", "переустановка ос", "переустановить ос",
        "переустановка windows", "переустановить windows", "переустановить виндовс",
        "переустановка винды", "переустановить винду", "надо переустановить ос",
    ])
    ...
    is_general_pc_lag = any(w in t for w in [
        ...
        "медленно грузит", "долго грузится", "очень долго", "все грузится", "включается по",
        "программы могут загружаться", "компьютер медленно", "сильно виснет",
    ])

    # Проверка на явные сбои оргтехники / МФУ / сканирования (03 приоритетнее 1С)
    is_mfu_printer_issue = any(w in t for w in [
        "не сканирует", "скан документ", "сканирован", "мфу", "картридж", "замятие",
        "принтер не", "не печатает принтер", "печать на пол листа", "полосит", "тонер"
    ...
    if not is_general_pc_lag and not is_mfu_printer_issue and not is_network_infra_issue and not is_os_reinstall:
    ...
    # 10. Оргтехника, принтеры, ПК, аппаратный ремонт, общие тормоза, ОС (03)
    if not is_software_install_request and (
        is_general_pc_lag or is_os_reinstall or is_usb_peripheral or any(w in t for w in [
...
class ServiceRedirectRule(BaseRule):
    ...
    def evaluate(
        ...
        is_os_reinstall_ctx = any(w in full_text_lower for w in [
            "переустановка операционной системы", "переустановка ос", "переустановить ос",
            "переустановка windows", "переустановить windows", "переустановить виндовс",
            "переустановка винды", "переустановить винду", "надо переустановить ос",
        ])
        is_general_pc_lag_ctx = any(w in full_text_lower for w in [
            "компьютер работает медленно", "медленно работает сам компьютер",
            "тормозит компьютер", "зависает компьютер", "зависает пк", "зависает весь пк",
            "принт скрин не", "скриншот не", "не сохраняет скрин", "не сохраняет выделенный",
            "зависает весь", "виснет пк", "виснет компьютер", "невозможно работать,тормозит",
        ...
        is_network_infra_ctx = any(w in full_text_lower for w in NETWORK_INFRA_KEYWORDS)
        ...
        if current_root != target_root or current_root == "11":
            ...
            # Запросы на переустановку ОС обслуживаются 1-й линией без отмены:
            if is_os_reinstall_ctx and current_root in ("02", "03", "11"):
                return None

            # Заявки из общих вопросов (11), относящиеся к компетенции 1-й линии (ПК, лаги, сеть):
            if current_root == "11" and (is_general_pc_lag_ctx or is_network_infra_ctx) and target_root in ("02", "03", "04"):
                return None
