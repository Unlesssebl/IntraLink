...
class PhysicalDeliveryRule(BaseRule):
    ...
    def evaluate(
        ...
        has_future_promise = any(w in user_text for w in [
            "принесу", "привезем", "принесем", "привезу", "занесу", "доставлю", "сдам"
        ])
        if has_future_promise:
            # Если заявитель только обещает принести, подтверждение должно быть в комментариях
            is_already_delivered = any(w in comments_text for w in [
                "находится в 112", "принес в 112", "принесла в 112", "принесли в 112",
                "занес в 112", "оставил в 112", "уже в 112", "передал в 112", "принят в 112",
                "стоит в 112", "лежит в 112"
            ])
        else:
