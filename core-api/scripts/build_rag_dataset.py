...
async def fetch_task_comments_safe(auth_b64: str, task_id: int, semaphore: asyncio.Semaphore) -> tuple[int, list[dict] | None]:
    ...
    async with semaphore:
        try:
            ...
            logger.error("Ошибка при получении комментариев для задачи %s в параллельном запросе: %s", task_id, e)
            return task_id, None
...
async def main():
    ...
    if service_login and service_password:
        ...
        if not auth_b64:
            logger.error("Не удалось авторизоваться в IntraService с использованием логина/пароля!")
            sys.exit(1)
    ...
    try:
        for status in statuses:
            ...
            logger.info("Начинаем сбор задач для статуса %d, начиная со страницы %d", status, page)
            ...
            while new_processed_count < limit_tasks:
                ...
                if not tasks:
                    logger.info("Страница %d для статуса %d пуста. Переходим к следующему статусу.", page, status)
                    break
                ...
                logger.info("Параллельный опрос комментариев для %d подходящих задач...", len(tasks_to_process))
                comments_semaphore = asyncio.Semaphore(5)  # Ограничиваем concurrency до 5 запросов к API
                ...
                for task in tasks_to_process:
                    ...
                    if not valid_comments:
                        logger.info("Заявка %s не содержит комментариев. Пропускаем.", task_id)
                    ...
                    if not has_engineer_comment(task, valid_comments):
                        logger.info("Заявка %s не содержит комментариев специалистов техподдержки. Пропускаем.", task_id)
                    ...
                    logger.info("Успешно извлечена запись для заявки %s. Проблема: %s", task_id, kb_entry.problem)
        ...
        logger.info("Всего проверено заявок (постранично): %d", stats["total_inspected"])
        ...
        logger.info("Пропущено без ответа инженера: %d", stats["skipped_no_engineer_comments"])
        logger.info("Пропущено пустых/коротких решений: %d", stats["skipped_meaningless_solution"])
        logger.info("Сбоев запросов LLM API: %d", stats["llm_errors"])
        logger.info("Всего в чекпоинте: успешно=%d, отсеяно=%d", len(processed_task_ids), len(skipped_task_ids))
