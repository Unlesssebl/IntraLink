...
async def main():
    ...
    if service_login and service_password:
        ...
        if not auth_b64:
            logger.error(
                "Не удалось авторизоваться в IntraService с использованием настроек .env!"
            )
            sys.exit(1)
    ...
    try:
        ...
        closed_data = await intraservice.get_tasks_by_status(
            auth_b64, status_id=28, page=1, page_size=15
        )
        ...
        cancelled_data = await intraservice.get_tasks_by_status(
            auth_b64, status_id=30, page=1, page_size=15
        )
    ...
    for task in test_tasks:
        ...
        status_name = (
            "Закрыта"
            if status_id == 28
            else ("Отменена" if status_id == 30 else f"Статус {status_id}")
        )
        service_id = task.get("ServiceId")
        service_name = (
            services_map.get(service_id) or task.get("ServiceName") or "Не указан"
        )

        logger.info(
            "Тестируем задачу #%s (%s), раздел: %s", task_id, status_name, service_name
        )
        ...
        try:
            ...
            logger.info(
                "Результат: %s. Прогноз: %s -> %s (Reason: %s)",
                evaluation,
                result.action,
    ...
    logger.info(
        "Правильно определено 'Оставить' (True Negative): %d", stats["none_correct"]
    )
    logger.info(
        "Ложное перенаправление 'Redirect' (False Positive): %d",
        stats["none_incorrect"],
    )
    logger.info(
        "Правильно перенаправлено 'Redirect' (True Positive): %d",
        stats["redirect_correct"],
    )
    logger.info(
        "Пропущено перенаправление 'None' (False Negative): %d",
        stats["redirect_incorrect"],
    )
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        json.dump(
            {"stats": stats, "accuracy": accuracy, "results": results_log},
            f,
            ensure_ascii=False,
            indent=2,
        )
