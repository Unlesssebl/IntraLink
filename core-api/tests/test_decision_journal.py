...
async def test_feedback_recording_and_serialization():
    async with AsyncSessionLocal() as db:
        ...
        record = await journal.record_triage(
            ...
            task=task(94004),
            history=[],
            decision={"rule_type": "printer_install", "comment": "Предложение", "status_id": 29},
        )
        ...
        assert feedback.final_action_json == {"status_id": 29, "comment": "Выполнено вручную"}
        ...
        feedback_list = list(
            (
                await db.scalars(
                    select(DecisionFeedback).where(DecisionFeedback.decision_id == record.id)
                )
            ).all()
        )
...
async def test_decision_api_rbac_and_flow():
    ...
    async with AsyncSessionLocal() as db:
        ...
        record = await journal.record_triage(
            ...
            task=task(94005),
            history=[],
            decision={"rule_type": "printer_install", "comment": "API test", "status_id": 29},
        )

    operator_token = await make_access_token("test.decision.operator", "helpdesk_operator")
