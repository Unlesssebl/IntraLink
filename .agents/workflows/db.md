...
## Шаги выполнения:
1. **Проверка:** Выполнить `uv run python helpdesk_tool.py check-db` в каталоге `helpdesk_agent/`.
2. **Автозапуск:** При недоступности контейнера выполнить `uv run python helpdesk_tool.py start-db` и дождаться готовности.
