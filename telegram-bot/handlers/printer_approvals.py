...
def get_approval_keyboard(task_id: int) -> InlineKeyboardMarkup:
    buttons = [
        [
            InlineKeyboardButton(
                text="Подтвердить автоустановку",
                callback_data=f"printer_approve:{task_id}",
            )
        ],
        [
            InlineKeyboardButton(
                text="Отклонить (установка вручную)",
                callback_data=f"printer_reject:{task_id}",
            )
        ],
        [
            InlineKeyboardButton(
                text="Изменить параметры", callback_data=f"printer_edit:{task_id}"
            )
        ],
        [
            InlineKeyboardButton(
                text="Запросить действие у пользователя",
                callback_data=f"printer_ask_user:{task_id}",
            )
        ],
    ]
...
def get_start_keyboard(task_id: int) -> InlineKeyboardMarkup:
    buttons = [
        [
            InlineKeyboardButton(
                text="Запустить установку", callback_data=f"printer_start:{task_id}"
            )
        ],
        [InlineKeyboardButton(text="Отменить", callback_data="printer_menu_close")],
    ]
    return InlineKeyboardMarkup(inline_keyboard=buttons)
...
async def cmd_printers_menu(message: Message, state: FSMContext):
    ...
    if tasks:
        for task in tasks:
            ...
            builder.button(
                text=btn_text, callback_data=PrinterTaskSelect(task_id=task["Id"])
            )
...
async def process_printer_task_select(
    callback: CallbackQuery, callback_data: PrinterTaskSelect
):
...
async def process_manual_task_id(message: Message, state: FSMContext):
    ...
    if not message.text or not message.text.isdigit():
        await message.answer(
            "⚠️ Ошибка: Номер заявки должен содержать только цифры. Попробуйте еще раз:"
        )
        return
...
async def process_ask_user_button(callback: CallbackQuery):
    ...
    await _publish_response(
        task_id, action="ask_user", tg_user_id=callback.from_user.id
    )
    if isinstance(callback.message, Message):
        await callback.message.edit_text(
...
async def process_connection(message: Message, state: FSMContext):
    ...
    update_kwargs = {}
    if "target_pc" in data:
        update_kwargs["target_pc"] = data["target_pc"]
    if "model_key" in data:
        update_kwargs["model_key"] = data["model_key"]
    if "connection_type" in data:
        update_kwargs["connection_type"] = data["connection_type"]

    if not message.from_user:
        return
    await _publish_response(
        task_id, action="update", tg_user_id=message.from_user.id, **update_kwargs
    )
    await message.answer(
        f"Параметры для задачи #{task_id} обновлены и отправлены воркеру."
    )
    await state.clear()
