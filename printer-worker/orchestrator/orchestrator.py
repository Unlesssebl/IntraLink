        ...
        try:
            if job.state != JobState.WAITING_APPROVAL:
                ...
                if errors:
                    if len(errors) == 2:
                        error_msg = (
                            f"Не вижу в сети ни ПК {job.target_pc}, ни МФУ {job.printer_address}.\n\n"
                            "Пожалуйста, проверьте оба устройства:\n"
                            "1. Убедитесь, что имя ПК и IP-адрес принтера указаны верно;\n"
                            "2. Проверьте подключение сетевых кабелей;\n"
                            "3. Перезагрузите компьютер и МФУ.\n\n"
                            "По вопросам звоните на номер 49-87."
                        )
                    elif not is_pc_reachable:
                        error_msg = (
                            f"Не вижу ПК {job.target_pc} в сети.\n"
                        ...
                        error_msg = (
                            f"Не вижу МФУ {job.printer_address} в сети.\n"
                            "1. Убедитесь в корректности IP-адреса принтера;\n"
                            "2. Перезагрузите МФУ;\n"
                            "3. Проверьте подключение сетевого кабеля.\n"
                            "По вопросам звоните на номер 49-87."
                        )
