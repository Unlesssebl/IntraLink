...
def parse_snmp_response(data: bytes) -> Optional[str]:
    try:
        ...
        pos = idx + 2 # Skip 0xa2 and length
...
async def resolve_hostname(hostname: str, timeout: float = 1.0) -> Optional[str]:
    try:
        return await asyncio.wait_for(
            asyncio.to_thread(resolve_hostname_sync, hostname),
            timeout=timeout
        )
    except Exception:
        return None

def query_snmp_oid_sync(ip: str, oid: str, port: int = 161, community: str = "public", timeout: float = 1.5) -> Optional[str]:
...
async def query_snmp_oid(ip: str, oid: str, port: int = 161, community: str = "public", timeout: float = 1.5) -> Optional[str]:
    return await asyncio.to_thread(query_snmp_oid_sync, ip, oid, port, community, timeout)

async def probe_printer_model(ip_or_host: str, community: str = "public", timeout: float = 1.5) -> Optional[str]:
    """
    ...
    if not ip:
        logger.warning("SNMP: Не удалось разрешить имя хоста '%s' или превышен таймаут", ip_or_host)
        return None

    # 2. Опрос OID
    model = await query_snmp_oid(ip, OID_HR_DEVICE_DESCR, community=community, timeout=timeout)
    if model:
        model = model.strip()
        logger.info("SNMP: hrDeviceDescr вернул '%s' для %s", model, ip_or_host)
        return model
        
    model = await query_snmp_oid(ip, OID_SYS_DESCR, community=community, timeout=timeout)
    if model:
        model = model.strip()
        logger.info("SNMP: sysDescr вернул '%s' для %s", model, ip_or_host)
        return model
        
    logger.warning("SNMP-автоопределение модели принтера на %s не дало результатов", ip_or_host)
    return None
