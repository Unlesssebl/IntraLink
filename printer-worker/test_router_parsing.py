...
async def test_router_parsing():
    ...
    assert kb.printer_name_prefixes == ["ittp", "kzmp", "kmkp"], "Неверные префиксы в БЗ"
    ...
    for text, expected in test_cases:
        ...
        assert result == expected, f"Ошибка парсинга для текста: {text!r}. Ожидалось {expected!r}, получено {result!r}"
    ...
    assert routed_job.printer_address == "ittp0024", f"Неверный printer_address: {routed_job.printer_address}"
    assert routed_job.driver_info is not None, "Драйвер не найден"
    assert routed_job.driver_info.model_key == "kyocera_ecosys_m2040dn", "Неверный драйвер"
    ...
    with patch("orchestrator.snmp.probe_printer_model") as mock_probe:
        ...
        print(f"Результат SNMP Auto-Discovery:")
        ...
        assert routed_job_snmp.model_key == "kyocera_ecosys_m2040dn", f"Неверная модель: {routed_job_snmp.model_key}"
        assert routed_job_snmp.connection_type == "tcpip", f"Неверный тип подключения: {routed_job_snmp.connection_type}"
