...
async def test_make_request_success():
    ...
    with patch.object(session, "request", return_value=mock_request) as mock_req_method:
        res = await api_client._make_request("test-endpoint", method="GET", params={"param": 1})
...
async def test_get_task_details():
    with patch("worker_services.api_client._make_request", new_callable=AsyncMock) as mock_make:
