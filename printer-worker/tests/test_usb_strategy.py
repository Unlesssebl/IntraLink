...
async def test_usb_probe_printer_not_found():
    ...
    with patch("strategies.usb_strategy.winrm_executor.run_powershell", new_callable=AsyncMock) as mock_ps:
...
async def test_usb_probe_success(mock_kb):
    ...
    with patch("strategies.usb_strategy.winrm_executor.run_powershell", new_callable=AsyncMock) as mock_ps:
        ...
        mock_kb.find_by_hw_id.assert_called_once_with("USBPRINT\\HEWLETT-PACKARDHP_LAFFCC")
...
async def test_usb_execute_success(mock_kb):
    ...
    with (
        patch("strategies.usb_strategy.smb_executor.check_source_accessible", new_callable=AsyncMock, return_value=True),
        patch("strategies.usb_strategy.smb_executor.copy_driver_to_temp", new_callable=AsyncMock, return_value=True),
        patch("strategies.usb_strategy.winrm_executor.run_powershell", new_callable=AsyncMock) as mock_ps,
        patch("worker_services.redis_listener.save_job_state", new_callable=AsyncMock) as mock_save_state
    ):
...
async def test_usb_execute_source_inaccessible(mock_kb):
    strategy = UsbDiscoveryStrategy(kb=mock_kb)
    driver = mock_kb.find_by_hw_id.return_value
    job = PrintJob(task_id=101, tg_user_id=222, raw_text="USB", target_pc="PC-TEST", driver_info=driver)
    
    with (
        patch("strategies.usb_strategy.smb_executor.check_source_accessible", new_callable=AsyncMock, return_value=False),
...
async def test_usb_execute_copy_failure(mock_kb):
    ...
    with (
        patch("strategies.usb_strategy.smb_executor.check_source_accessible", new_callable=AsyncMock, return_value=True),
        patch("strategies.usb_strategy.smb_executor.copy_driver_to_temp", new_callable=AsyncMock, return_value=False),
        patch("worker_services.redis_listener.save_job_state", new_callable=AsyncMock)
    ):
        res = await strategy.execute(job)
        assert res.state == JobState.FAILED
