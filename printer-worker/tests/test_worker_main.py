...
async def test_start_heartbeat():
    ...
    with (
        patch(
            "worker_services.redis_listener.get_redis", return_value=mock_redis_client
        ),
        patch(
        ...
        mock_redis_client.setex.assert_called_once_with(
            "printer_worker:status", 10, "online"
        )
    ...
    with patch(
        ...
        try:
            ...
            mock_redis_client.expire.assert_called_once_with(
                "printer_job_logs_history:9999", 2592000
            )
...
async def test_main_lifecycle():
    ...
    with (
        ...
        patch(
            "worker_main.start_redis_listener", side_effect=dummy_task
        ) as mock_listener,
