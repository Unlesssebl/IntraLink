...
async def test_winrm_run_powershell_success():
    ...
    with patch("executors.winrm_executor.winrm.Session", return_value=mock_session):
        status, stdout, stderr = await executor.run_powershell("target-pc", "Write-Output 'Hello'")
        assert status == 0
...
async def test_winrm_run_powershell_retry_then_success():
    ...
    with (
        ...
        status, stdout, stderr = await executor.run_powershell("target-pc", "Write-Output 'Retry'")
        assert status == 0
...
async def test_winrm_run_powershell_failure_all_attempts():
    ...
    mock_session = MagicMock()
    mock_session.run_ps.side_effect = requests.exceptions.RequestException("Connection timeout")
    
    with (
        patch("executors.winrm_executor.winrm.Session", return_value=mock_session),
        patch("time.sleep") as mock_sleep
    ):
        status, stdout, stderr = await executor.run_powershell("target-pc", "Write-Output 'Fail'")
        assert status == -1
...
async def test_winrm_run_powershell_timeout():
    ...
    with patch("asyncio.to_thread", side_effect=slow_thread):
        status, stdout, stderr = await executor.run_powershell("target-pc", "Write-Output 'Slow'", timeout=0.1)
        assert status == -1
