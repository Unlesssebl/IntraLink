...
def test_copy_dir_smb():
    ...
    mock_listdir = MagicMock(side_effect=[
    ...
    mock_stat = MagicMock(side_effect=[
...
async def test_copy_driver_to_temp_success():
    ...
    with (
        ...
        res = await executor.copy_driver_to_temp(
            src="\\\\srv\\drivers\\model\\driver.inf",
            dest_host="target-pc"
        )
        assert res is True
...
async def test_check_source_accessible_success():
    ...
    with (
        ...
        res = await executor.check_source_accessible("\\\\srv\\drivers\\model\\driver.inf")
        assert res is True
