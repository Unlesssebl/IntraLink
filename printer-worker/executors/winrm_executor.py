...
class WinRMExecutor:
    ...
    async def run_powershell(self, target_pc: str, script: str) -> Tuple[int, str, str]:
        """
        Асинхронный запуск PowerShell скрипта на целевом компьютере.
        """
        logger.info("WinRM [PowerShell] -> %s: запуск команды", target_pc)
        status_code, stdout, stderr = await asyncio.to_thread(self._run_ps_sync, target_pc, script)
        if status_code != 0:
