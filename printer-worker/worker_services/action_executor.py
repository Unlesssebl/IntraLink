...
from action_config import ACTIONS, ActionRule, ERROR_RULES
...
async def execute_action(action_name: str, job: PrintJob, error_detail: str = "") -> None:
    ...
    # Обновление статуса
    if rule.status_id is not None:
        try:
            await update_task_status(job.tg_user_id, job.task_id, rule.status_id)
