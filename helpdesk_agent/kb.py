...
def get_local_embed_model():
    ...
    if _fastembed_model is None:
        try:
            ...
            logger.warning("Ошибка инициализации fastembed: %s", e)
...
# ---------------------------------------------------------------------------
# 1. Генерация эмбеддингов (Gemini API -> FastEmbed Local)
# ---------------------------------------------------------------------------

async def get_gemini_embedding(session: aiohttp.ClientSession, text_input: str) -> list[float] | None:
    """Генерирует вектор через Google Gemini API."""
    if not GEMINI_KEY:
        return None
...
def get_fastembed_vector(text_input: str) -> list[float]:
    ...
    if model is not None:
        try:
            vectors = list(model.embed([text_input]))
            if vectors:
...
async def get_hybrid_embedding(text_input: str) -> list[float]:
    """
    Гибридный генератор: пытается получить вектор через Gemini API,
    а при сбое или отсутствии сети мгновенно переключается на fastembed.
    """
    clean_text = clean_html(text_input).strip()
    if not clean_text:
        return [0.0] * 384

    # 1. Попытка Gemini API (если есть ключ)
    if GEMINI_KEY:
        try:
            connector = aiohttp.TCPConnector(ssl=False)
            async with aiohttp.ClientSession(connector=connector) as session:
                vec = await get_gemini_embedding(session, clean_text)
                if vec:
                    return vec
        except Exception:
            pass

    # 2. Быстрый и надежный локальный fastembed
    return get_fastembed_vector(clean_text)
...
# ---------------------------------------------------------------------------
# 2. Хранилище базы знаний (JSON Cache + PostgreSQL)
# ---------------------------------------------------------------------------
...
async def test_db_connection() -> bool:
    """Проверяет доступность PostgreSQL или локального хранилища."""
    try:
        import asyncpg
        conn = await asyncpg.connect(
            user="Unless", password="Vimeworld@", host="127.0.0.1", port=5432, database="smart_contacts", timeout=2
        )
        await conn.close()
        return True
    except Exception:
...
def is_trivial_solution(text_val: str) -> bool:
    """Определяет, является ли решение пустым/тривиальным."""
...
def smart_filter_task(task: dict[str, Any], comments: list[dict[str, Any]]) -> tuple[bool, str, dict[str, Any]]:
    ...
    # Извлекаем все содержательные комментарии специалистов
    engineer_comments = []
    for c in comments:
...
# ---------------------------------------------------------------------------
# 4. Векторный поиск и индексация
# ---------------------------------------------------------------------------
...
async def search_knowledge_base(
    ...
    """
    Выполняет семантический векторный поиск по базе знаний.
    """
    if not query_text.strip():
        return []

    query_vector = await get_hybrid_embedding(query_text)
    ...
    for str_id, item in kb_data.items():
        ...
        if dist <= distance_threshold:
            # Масштабируем сходство в понятные проценты (0.35 -> 65%, 0.60 -> 85%, 0.80+ -> 95%+)
...
async def index_task_record(
    ...
    """Индексирует решение заявки в базу знаний."""
    try:
        embed_input = f"Тема: {original_name}\nПроблема: {problem}\nРешение: {solution}"
        vec = await get_hybrid_embedding(embed_input)
        ...
        logger.error("Ошибка индексации заявки #%s в базу знаний: %s", task_id, e)
        return False


# ---------------------------------------------------------------------------
# 5. Пакетная синхронизация (Sync History KB)
# ---------------------------------------------------------------------------
...
async def sync_history_kb(
    ...
    """
    stats = {"fetched": 0, "accepted": 0, "skipped": 0, "indexed": 0, "reasons": {}}
    print(f"Запрос выполненных (29) и отмененных (30) заявок из IntraService...")
    ...
    for t in tasks:
        ...
        # Получаем комментарии из карточки или из истории изменений
